...
namespace DuAnBanGiay
{
    public partial class UCSanPham : UserControl
    {
        ...
        public UCSanPham()
        {
            ...
            btnThemChatLieu.Click += (_, _) => AddChatLieu();
            ...
            dataGridView1.RowPostPaint += (_, e) => PaintStt(dataGridView1, e);
            dgvBienThe.RowPostPaint += (_, e) => PaintStt(dgvBienThe, e);
        }
        ...
        private void SetupGrid()
        {
            ...
            dataGridView1.RowHeadersWidth = 60; // STT
        }
        ...
        private void SetupBienTheGrid()
        {
            ...
            dgvBienThe.RowHeadersWidth = 60; // STT
        }
        ...
        private void LoadBienTheFromSelectedSanPham()
        {
            ...
            if (dgvBienThe.Columns["TenSP"] is not null) dgvBienThe.Columns["TenSP"].HeaderText = "Tên SP";
        ...
        private static void PaintStt(DataGridView dgv, DataGridViewRowPostPaintEventArgs e)
        {
            var stt = (e.RowIndex + 1).ToString();
            var rect = new Rectangle(e.RowBounds.Left, e.RowBounds.Top, dgv.RowHeadersWidth, e.RowBounds.Height);
        ...
        {
            var ten = (Interaction.InputBox("Nhập tên chất liệu mới:", "Thêm chất liệu", "") ?? "").Trim();
            if (string.IsNullOrWhiteSpace(ten)) return;

            try
            {
                var id = _danhMucRepo.InsertChatLieu(ten);
                cbChatLieu.DataSource = _danhMucRepo.GetChatLieu();
                cbChatLieu.DisplayMember = nameof(DanhMucItem.Name);
                cbChatLieu.ValueMember = nameof(DanhMucItem.Id);
                cbChatLieu.SelectedValue = id;
            }
            catch (Exception ex)
            {
                MessageBox.Show(
